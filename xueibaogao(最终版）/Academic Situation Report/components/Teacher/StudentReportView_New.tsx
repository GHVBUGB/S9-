...
import { BookOpen, Clock, Target, MapPin, Lock, Zap, Brain, Layout, Scissors, Play, Eye } from 'lucide-react';
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-black font-serif text-gray-900 mb-1">学生学情报告</h1>
            <p className="text-sm text-gray-500 font-medium">{lesson.date}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-200">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500">学生ID: {student.id}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5 border border-blue-200">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-gray-600 uppercase">阅读篇数</span>
            </div>
            <div className="text-3xl font-black text-gray-900">{lesson.articlesCount}</div>
            <div className="text-[10px] text-gray-500 mt-1">积少成多</div>
          </div>
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-5 border border-purple-200">
            <div className="flex items-center gap-2 mb-3">
              <Clock className="w-5 h-5 text-purple-600" />
              <span className="text-xs font-bold text-gray-600 uppercase">专注时长</span>
            </div>
            <div className="text-3xl font-black text-gray-900">{lesson.focusDuration}min</div>
            <div className="text-[10px] text-gray-500 mt-1">有效思考时长</div>
          </div>
          <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-5 border border-orange-200">
            <div className="flex items-center gap-2 mb-3">
              <BookOpen className="w-5 h-5 text-orange-600" />
              <span className="text-xs font-bold text-gray-600 uppercase">阅读字数</span>
            </div>
            <div className="text-3xl font-black text-gray-900">{lesson.wordsRead}</div>
            <div className="text-[10px] text-gray-500 mt-1">≈ 1/5 章《哈利波特》</div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-1 text-gray-500">
              ...
              <span className="text-xs font-bold">{lesson.vocabulary.length}词</span>
            </div>
          </div>
      ...
      {lesson.sentences && lesson.sentences.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-xs font-bold text-gray-400">完成 {lesson.sentences.length} 例，点击展开</div>
          </div>
          <div className="space-y-3">
              ...
              <div key={idx} onClick={() => setSelectedSentence(sentence)} className="bg-gray-50 rounded-xl p-4 border border-gray-100 hover:bg-gray-100 transition-colors cursor-pointer">
                <div className="flex justify-between items-start gap-4 mb-2">
                  ...
                  <div className={`shrink-0 text-[9px] font-bold px-2 py-0.5 rounded border ${
                    ...
                    sentence.difficulty === 'medium' ? 'bg-yellow-50 text-yellow-600 border-yellow-100' :
      ...
      {lesson.subjectPassport && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-black font-serif">主题护照</h2>
            <div className="px-3 py-1 bg-blue-600 text-white rounded-full text-xs font-bold uppercase">
              {lesson.unit}
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="text-center mb-6">
          <span className="text-xs font-bold tracking-[0.2em] text-blue-600 uppercase block mb-1">五维能力图谱</span>
          <h2 className="text-xl font-black font-serif text-black">综合阅读能力雷达</h2>
        </div>
      ...
      {periodData.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-lg font-black font-serif mb-1">周期对比</h2>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">期间比较</p>
            </div>
            <div className="bg-gray-100 p-1 rounded-full flex">
              <button 
                onClick={() => setPeriodType('week')}
                className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${periodType === 'week' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}
              >
                本周与上周
              </button>
              <button 
                onClick={() => setPeriodType('month')}
                className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${periodType === 'month' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}
              >
                本月与上月
              </button>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {periodData.map((item, idx) => (
              <div key={idx} className="bg-gray-50 rounded-xl p-4 border border-gray-100 flex flex-col justify-between group hover:border-blue-200 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <span className="text-xs font-bold text-gray-400">{item.label}</span>
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${
                    (item.reverseColor ? item.change < 0 : item.change > 0) 
                ...
                <div className="pt-2 border-t border-gray-100 flex justify-between items-center text-xs font-bold text-gray-400">
                  <span>上期: {item.prevValue}</span>
                  <span className={`${(item.reverseColor ? item.diff < 0 : item.diff > 0) ? 'text-green-600' : 'text-gray-400'}`}>
